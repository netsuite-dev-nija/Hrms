...
define(["N/record", "N/log", './moment.js', 'N/format'], function (record, log, moment, format) {
 ...
 /*  function saveRecord(scriptContext) {
   ...
   function saveRecord(scriptContext) {
        try {
            var currentObjRecord = scriptContext.currentRecord;
            var employeeId = currentObjRecord.id; // Get internal ID to exclude from duplicate search
            
            var customform = currentObjRecord.getValue({
                fieldId: 'customform'
            });

            // Only run logic for specific form
            if (customform == 470) {
                var accessmobile = currentObjRecord.getValue({
                    fieldId: 'custentity_hris_emp_accesstomobile'
                });
                ...
                if (accessmobile == true) {
                    ...
                    var duplicateSearch = search.create({
                        ...
                        filters: [
                            ['custentity_hris_empmobileemail', 'is', empemail],
                            'AND',
                            ['isinactive', 'is', 'F'] // Only check active employees
                        ],
                        columns: ['entityid']
                    });

                    // If we are editing an existing record, exclude the current ID from search
                    if (employeeId) {
                        duplicateSearch.filterExpression = [
                            ['custentity_hris_empmobileemail', 'is', empemail],
                            'AND',
                            ['internalid', 'noneof', employeeId],
                            'AND',
                            ['isinactive', 'is', 'F']
                        ];
                    }
                    ...
                    if (searchResultCount > 0) {
                        alert('Duplication Error: The Mobile Email "' + empemail + '" is already assigned to another employee.');
            ...
            return true; // Usually allow save if script fails unless logic is critical
        }
    }
