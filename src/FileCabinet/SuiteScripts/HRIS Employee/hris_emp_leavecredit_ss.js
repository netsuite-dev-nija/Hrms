function SCH_LeaveCreditOnJoining() {
  ...
  var emp_category = employeeData.custentity_hris_empcategory;
  nlapiLogExecution("DEBUG","emp_category",emp_category)
...
function searchLeaveCreditConfigRecords(
  ...
  try {
    ...
    filters.push(
      new nlobjSearchFilter("custrecord_hris_lveconfig_grade", null, "anyof", emp_grade)
    );
    filters.push(
      new nlobjSearchFilter(
    ...
    filters.push(
      new nlobjSearchFilter("custrecord_hris_lveconfig_gender", null, "anyof", empGender)
    );
    filters.push(
      new nlobjSearchFilter(
    ...
    filters.push(
      new nlobjSearchFilter(
        "custrecord_hris_lvecfg_creditseqno",
        null,
        "notequalto",
        4
      )
    ...
    filters.push(
      new nlobjSearchFilter(
        "custrecord_hris_lveconfig_excluautobalan", // corrected field name
        null,
        "is",
        "F"
      )
 ...
 columns.push(
    ...
    if (searchresults != null) {
      ...
      for (var i = 0; i < searchresults.length; i++) {
        ...
        var Leavecreditfreq=searchresults[i].getValue("custrecord_hris_lvecfg_creditseqno")||'';
        nlapiLogExecution("DEBUG", "Leave Credit freq",Leavecreditfreq)
        ...
        var rounded_off_type = o_leaveCreditConfig.getFieldValue(
          "custrecord_hris_lvecfg_round_off"
        );
        ...
        var proRataCheck = o_leaveCreditConfig.getFieldValue(
          "custrecord_hris_lvecfg_pro_rata"
        );
        var employeecategory = o_leaveCreditConfig.getFieldValue(
          "custrecord_hris_lvecfg_employee_catagory"
        );
       
        var accuraldays = o_leaveCreditConfig.getFieldValue(
          "custrecord_hris_lvecfg_accuraldays"
        );
        var roundOffId = "";
        var emp_categoryrecord = nlapiLoadRecord(
          "customrecord_hris_employeecategory",
          emp_category
        );
        var emp_category_seqno = emp_categoryrecord.getFieldValue(
          "custrecord_hris_lvecfg_accuraldays"
        );
        if (proRataCheck == "T") {
        ...
        if (CreditFreq == 2) {
          if (Leavecreditfreq != 3) {
            ...
            nlapiLogExecution("DEBUG", "Days Count : ", days_count);
            ...
            //changes in leave credit
            final_days_to_credit = NoOfDaysCredited;
          }
          else {

            var current_date = new Date();
            nlapiLogExecution('AUDIT','Current Date',+current_date);
     
           // var startDate = new Date(current_date);
          //  nlapiLogExecution('AUDIT','StartDate',+startDate);
         //   var endDate = new Date(empHireDateObj);
         ...
         //   nlapiLogExecution('AUDIT','EndDate',+endDate);
           ...
           // var timeDifference = endDate.getTime() - startDate.getTime();
            ...
            var dayDifference = date_difference(current_date,empHireDateObj);
            // Output the difference in days
            nlapiLogExecution('AUDIT', 'Date Difference', 'The difference in days is: ' + dayDifference);
//Comment by florence it takes the accural days from the field value
          //  var finalAnnual = dayDifference * 0.08219
          /* if(emp_category_seqno==1){
             var finalAnnual = dayDifference * accuraldays
          }
          else{
            var finalAnnual = dayDifference * accuraldays
          } */
            nlapiLogExecution('AUDIT','accuraldays',accuraldays);
          var finalAnnual = dayDifference * (accuraldays/30)
            nlapiLogExecution('AUDIT', 'Final Annual', +finalAnnual);
        ...
        if (empHireDateObj) {
          ...
          columns2.push(
            new nlobjSearchColumn("custrecord_hris_lvecfg_noofdaysto_credit")
          );
          ...
          if (searchresults2) {
            ...
            for (var j = 0; j < searchresults2.length; j++) {
              ...
              if (!LeaveBalSearchResults3) {
                ...
                // Changed according to vanitha mam for sick leave no of days to be credited
                nlapiLogExecution(
                  "DEBUG",
                  "parseFloat(_days)",
                  parseFloat(_days)
...
function formatDate(dateString) {
  ...
  var month = parts[1];
  var year = parts[2];
  // Reformat the date to M/D/YYYY format
...
function searchEmpInLeavebalance(
  ...
  if (
    ...
    try {
      ...
      // Added by Florence for Annual Leave
      //if(leaveType != 1){
      LeavebalanceRec.setFieldValue(
        "custrecord_hris_lvbal_leave_balance_cred",
